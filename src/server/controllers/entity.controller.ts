...
> {
	...
	// }: {
	// 	model: TPatchModel;
	// }): Partial<TEntity> & Pick<TEntity, 'id'>;
	...
	updateEntity(req: NextApiRequest, res: NextApiResponse): void {
		...
		try {
			if (this.validatePatchModel(model)) {
				...
				model.name = model.name.toLowerCase().replace(/[^a-zA-Z ]/g, '');
				const convertedModel: Partial<TEntity> & Pick<TEntity, 'id'> = this.getEntity(req, res);
				const updatedEntity = this.service.updateEntity({ updatedEntity: convertedModel });
				res.status(StatusCodes.OK).json(updatedEntity);
			}
		} catch (error) {
			res.status(StatusCodes.BAD_REQUEST).send(getErrorMessage({ error }));
		}
